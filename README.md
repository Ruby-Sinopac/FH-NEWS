...
# 2a) 只取指定指標，每個指標 = 一個工作表（列=機構、欄=16 期往右）
python -m fsc export --metrics "使用者人數,當月代理收付實質交易款項金額" \
       --label 機構名稱 --from 11401 --to 11504 --out 彙整.xlsx
...
| 參數 | 說明 |
|------|------|
| `--metrics` | 只取這些指標（逗號分隔），每個指標一個工作表。比對採「包含」，可容忍單位後綴。 |
| `--sheet-by` | 當作工作表名稱的欄位（如「類別」）。不給也不給 metrics 則用原始分頁名。 |
