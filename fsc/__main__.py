...
def _norm_period(p: str | None) -> str | None:
    """把 YYYY-MM 或民國代碼（11401）統一轉成 YYYY-MM。"""
    if not p:
        return None
    return parser.guess_period(p) or p


def cmd_columns(cfg: dict, *, period_from, period_to) -> None:
    """列出資料庫各分頁的欄位與範例值，協助選 --sheet-by / --label。"""
    db_path = cfg.get("db_path", "fsc_news.sqlite")
    exporter.list_columns(db_path, _norm_period(period_from), _norm_period(period_to))
...
def main(argv: list[str] | None = None) -> None:
    ...
    ap.add_argument("command",
                    choices=["run", "probe", "inspect", "crawl", "load",
                             "export", "columns"],
                    help="動作（export=整理成寬表 Excel；columns=列出欄位名稱；"
                         "probe=逐月診斷；inspect=檢視單一網址）")
